#!/usr/bin/env python3
"""Service entry point: get pixels on the screens as early as possible.

Imports only what is needed to drive the panels (config, display, lcd ->
...
otherwise spend looking at three black screens.
"""
...
def run():
    ...
    if not disp.splash():
        print("boot: no splash images in %s" % config.SPLASH_DIR)
    print("boot: panels up in %.2fs" % (time.monotonic() - t0))
    import main
    main.main(disp)
